    ...
    try:
        ...
        if bounds[6, 5] + bounds[6, 4] > 0:
            ...
            if bounds[6, 4] > 0:
                ...
                m.addConstr(
                    (quicksum(sw[n, d1, d2] * (bounds[6, 4] - 1 - d2) for n in N for d1 in Ds for d2 in
                              range(bounds[6, 4] - 1)) == 0), "minconswork"
                )
        ...
        if bounds[6, 3] + bounds[6, 2] > 0:
            ...
            if bounds[6, 3] > 0:
                m.addConstr(
                    (quicksum(tf[n, d1, d2] for n in N for d1 in D for d2 in range(bounds[6, 3], len(D))) == 0),
                    "maxconsfree"
                )
        ...
        if bounds[7, 5] + bounds[7, 4] > 0:
            ...
            if bounds[7, 5] > 0:
                m.addConstr(
                    (quicksum(tws[n, s1, s2] for n in N for s1 in S for s2 in range(bounds[7, 5], len(S))) == 0),
                    "maxconswork"
                )
            ...
            if bounds[7, 4] > 0:
                ...
                m.addConstr(
                    (quicksum(sws[n, s1, s2] * (bounds[7, 4] - 1 - s2) for n in N for s1 in Ss for s2 in
                              range(bounds[7, 4] - 1)) == 0), "minconswork"
                )
        ...
        if bounds[7, 3] + bounds[7, 2] > 0:
            ...
            if bounds[7, 3] > 0:
                m.addConstr(
                    (quicksum(tfs[n, s1, s2] for n in N for s1 in S for s2 in range(bounds[7, 3], len(S))) == 0),
                    "maxconsfree"
                )
            ...
            if bounds[7, 2] > 0:
                ...
                m.addConstr(
                    (quicksum(sfs[n, s1, s2] * (bounds[7, 2] - 1 - s2) for n in N for s1 in Ss for s2 in
                              range(bounds[7, 2] - 1)) == 0), "minconsfree"
                )
        ...
        if bounds[11, 5] + bounds[11, 4] > 0:
            ...
            if bounds[11, 5] > 0:
                m.addConstr(
                    (quicksum(tw1[n, d1, s, d2] for s in S for n in N for d1 in D for d2 in
                              range(bounds[11, 5], len(D))) == 0), "maxconssameshift"
                )
            ...
            if bounds[11, 4] > 0:
                ...
                m.addConstr(
                    (quicksum(sw1[n, d1, s, d2] * (bounds[11, 4] - 1 - d2) for s in S for n in N for d1 in Ds for d2 in
                              range(bounds[11, 4] - 1)) == 0), "minconssameshift"
                )
        ...
        if bounds[11, 3] + bounds[11, 2] > 0:
            ...
            if bounds[11, 3] > 0:
                m.addConstr(
                    (quicksum(tw1f[n, d1, s, d2] for n in N for s in S for d1 in D for d2 in
                              range(bounds[11, 3], len(D))) == 0), "maxconsfree"
                )
            ...
            if bounds[11, 2] > 0:
                ...
                m.addConstr(
                    (quicksum(sw1f[n, d1, s, d2] * (bounds[11, 2] - 1 - d2) for n in N for s in S for d1 in Ds for d2 in
                              range(bounds[11, 2] - 1)) == 0), "minconsw1free"
                )
